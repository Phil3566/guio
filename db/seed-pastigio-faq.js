...
const faqs = [
  ...
  },
  {
    ...
    answer: "This is the most common issue and usually fixable.\n\n1. **Check the signal type:** Your frame only works with 2.4GHz WiFi. If you see your WiFi name listed twice, tap the one **without** \"5G\" at the end.\n2. **Double-check the password:** Tap the eye icon next to the password field on the frame's screen to see what you're typing. Passwords are case-sensitive.\n3. **Restart both devices:** Unplug your frame for a minute, then plug it back in. For your router, unplug it for 30 seconds, then plug it back in and wait 2 minutes for it to fully restart.\n\n[TECHNICAL]The frame only supports 2.4GHz 802.11 b/g/n. It cannot see 5GHz networks. If your router uses band steering (single SSID for both bands), the frame may fail to connect. Ask your ISP to create a dedicated 2.4GHz SSID. Must be WPA2 (not WPA3), channels 1-11, and not hidden.[/TECHNICAL]"
  },
  {
    question: "What is 2.4GHz WiFi?",
    answer: "Think of it like an AM/FM radio — your router sends out two different WiFi signals, and your frame can only pick up one of them.\n\nThe one your frame needs is called **2.4GHz**. Most home routers send out both signals, so you probably have it already. The trick is making sure your frame connects to the right one.\n\n**Quick check:** If you see your WiFi name listed twice — one normal and one with \"5G\" at the end — tap the one **without** 5G. That's the one your frame needs.\n\nIf you only see one network name, a family member can split the WiFi into separate 2.4GHz and 5GHz networks in the router settings.\n\n[TECHNICAL]Most dual-band routers combine 2.4GHz and 5GHz under one SSID using band steering. To create a separate 2.4GHz SSID: log into the router admin panel (usually 192.168.1.1), find Wireless Settings, disable Smart Connect or Band Steering, and set a unique name for the 2.4GHz band.[/TECHNICAL]"
  },
  {
