...
class FaqCache {
  ...
  checkLimits(token, fingerprint) {
    ...
    if (session.request_count >= 50) return { allowed: false, reason: "session_limit" };

    // Fingerprint limit: 200 requests across all sessions from this device
    ...
    if (fpTotal >= 200) return { allowed: false, reason: "fingerprint_limit" };
