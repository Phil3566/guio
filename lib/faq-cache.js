...
class FaqCache {
  constructor(dbPath) {
    ...
    this._initSchema();
    // Load threshold: DB setting > env var > default 0.45
    const dbThreshold = this.db.prepare("SELECT value FROM settings WHERE key = ?").get("faq_similarity_threshold");
    this.threshold = dbThreshold ? parseFloat(dbThreshold.value) : (parseFloat(process.env.FAQ_SIMILARITY_THRESHOLD) || 0.45);
    this._seedFAQs();
    this._seedResources();
    this._loadCache();
    this._loadResources();
    this._backfillScores();
  }
  ...
  _initSchema() {
    ...
    // Migration: add score columns to request_log
    try {
      this.db.exec("ALTER TABLE request_log ADD COLUMN tri_score REAL");
      this.db.exec("ALTER TABLE request_log ADD COLUMN kw_score REAL");
      this.db.exec("ALTER TABLE request_log ADD COLUMN best_score REAL");
      this.db.exec("ALTER TABLE request_log ADD COLUMN matched_q TEXT");
    } catch (e) { /* columns already exist */ }
  }

  _backfillScores() {
    const rows = this.db.prepare("SELECT id, device_id, question FROM request_log WHERE best_score IS NULL").all();
    if (rows.length === 0) return;
    const update = this.db.prepare("UPDATE request_log SET tri_score = ?, kw_score = ?, best_score = ?, matched_q = ? WHERE id = ?");
    for (const row of rows) {
      const scores = this.scoreBest(row.device_id, row.question);
      update.run(scores.tri, scores.kw, scores.best, scores.matchedQ, row.id);
    }
    console.log(`Backfilled scores for ${rows.length} request_log entries`);
  }
  ...
  _seedFAQs() {
    const seeds = [
      ...
      },
      {
        ...
        aliases: [
          "WiFi not working on the frame",
          "Frame can't find my WiFi network",
          "How do I connect the frame to WiFi?",
          "WiFi setup for the picture frame",
          "The frame won't connect to the internet",
          "Frame keeps disconnecting from WiFi",
          "My WiFi doesn't show up on the frame",
          "How to connect Frameo to WiFi",
          "WiFi password not working on the frame",
          "Frame says no network found",
          "2.4GHz WiFi setup for Frameo",
          "Does this frame work with 5GHz WiFi?",
          "Frame is offline",
          "Can't get the frame online",
      ...
      },
      {
        ...
        aliases: [
          "Can I use the frame without paying?",
          "What do I get for free?",
          "What is the free version vs paid?",
          "Frameo subscription pricing",
        ],
        answer: "**No.** Your frame works fully without paying anything.\n\n**What's free:**\n- Receiving photos from family\n- Slideshow, sleep schedule, brightness controls\n- Backing up photos to a microSD card\n- Sending up to 10 photos at a time (just send more batches — no limit on total)\n- Videos up to 15 seconds\n\n**What Frameo+ adds** ($1.99/month or $16.99/year — pricing may vary, check the Frameo app):\n- Send up to 100 photos at once\n- Longer videos (over 15 seconds)\n- Cloud backup\n- Remote frame management from the app\n\n**Tip:** To load lots of photos without a subscription, use a microSD card (FAT32, 32GB max) — you can load hundreds at once.\n\nFor a full comparison, see [CROSSREF:commonq]Common Questions → Cost & Subscription[/CROSSREF]."
      },
      {
        ...
        aliases: [
          "Screen is cracked",
          "I dropped the frame and the glass broke",
          "Can I replace the screen?",
          "The screen cracked how do I fix it?",
          "Frame fell and screen is broken",
          "Broken screen on the picture frame",
      ...
      },
      {
        ...
        aliases: [
          "Does the frame play videos?",
          "How do I send a video to the picture frame?",
          "Can I put videos on the frame?",
          "Video playback on Frameo",
          "How long can videos be on the frame?",
          "Does it support video?",
          "Can the frame show video clips?",
          "How to send a movie to the frame",
        ],
      ...
      },
      {
        ...
        aliases: [
          "Not enough storage on the frame",
          "Frame ran out of space",
          "How do I free up space on the frame?",
          "Memory full on picture frame",
          "Can't receive new photos storage full",
          "How much storage does the frame have?",
          "How do I delete photos to make room?",
          "Frame memory is full what do I do?",
          "No more room for photos",
        ],
      ...
      },
      {
        ...
        aliases: [
          "How do I manage the frame remotely?",
          "Can I change settings from my phone?",
          "Remote control for the picture frame",
          "Can I delete photos from my phone?",
          "Is there a remote app for the frame?",
          "Can I manage the frame without being there?",
          "How to control Frameo from my phone",
        ],
      ...
      },
      {
        ...
        aliases: [
          ...
          "Can I put a message on a picture?",
          "How to add text to a photo on the frame",
          "Can I label photos on the frame?",
          "Adding a description to a photo",
          "Can I write something on the picture?",
          "How do captions work on Frameo?",
          "Can the frame show text with photos?",
        ],
      ...
      },
      {
        ...
        aliases: [
          "Does Frameo work on iPad?",
          "Can I send photos from my tablet?",
          "Is there a tablet version of the app?",
          "Frameo app for iPad",
          "Does the app work on Android tablet?",
          "Can I use my iPad to send photos?",
        ],
      ...
      },
      {
        ...
        aliases: [
          "Is my frame connected to the internet?",
          "How do I know if WiFi is working on the frame?",
          "Where is the WiFi indicator on the frame?",
          "How to check WiFi status on Frameo",
          "Is the frame online?",
          "How can I tell if the frame has WiFi?",
        ],
      ...
      },
      {
        ...
        aliases: [
          "What are the buttons on the screen?",
          "Explain the icons on the frame",
          "What does each icon do?",
          "What are the symbols on the frame?",
          "Menu icons explained",
          "What is the gear icon for?",
          "What do the little pictures on the screen mean?",
          "How do I use the touchscreen buttons?",
        ],
      ...
      },
      {
        ...
        aliases: [
          "How do I connect multiple frames?",
          "Can I send photos to more than one frame?",
          "Does the app work with two frames?",
          "How to add a second frame to the app",
          "Can one phone send to multiple frames?",
          "We have two frames can I use one app?",
          "Setting up multiple Frameo frames",
          "How many frames can I connect to?",
        ],
      ...
      },
      {
        ...
        aliases: [
          "Photos are cropped on the frame",
          "My picture is zoomed in too much",
          "The frame is cutting off parts of my photos",
          "Why can't I see the whole photo?",
          "Photos don't fit the screen properly",
          "Heads are cut off in photos",
          "The frame is trimming my pictures",
          "How to show the full photo on the frame",
        ],
      ...
      },
      {
        ...
        aliases: [
          "What does Fill mean on the frame?",
          "What does Fit mean on the frame?",
          "How do I change how photos are displayed?",
          "Fill frame setting explained",
          "How to stop photos from being cropped",
      ...
      },
      {
        ...
        aliases: [
          ...
          "Frame won't respond to touch",
          "I tap the screen but nothing happens",
          "The screen doesn't respond",
          "Touch stopped working on the frame",
          "How do I fix an unresponsive screen?",
          "Screen won't react to my finger",
          "The frame ignores my taps",
        ],
      ...
      },
      {
        ...
        aliases: [
          "How do I turn off sound on the frame?",
          "Frame makes noise at night",
          "How to mute the picture frame",
          "Can I turn off video sound?",
          "The frame plays audio while I'm sleeping",
          "How to set a sleep timer on the frame",
          "Can the frame turn off at night?",
          "Night mode for the picture frame",
          "How do I set the frame to sleep?",
          "Frame won't shut off at night",
        ],
    ...
    for (const s of seeds) {
      // Insert/update the primary question
      ...
      // Insert/update each alias with the same answer
      if (s.aliases) {
        for (const alias of s.aliases) {
          const aliasNorm = normalize(alias);
          if (aliasNorm === norm) continue; // skip if identical after normalization
          const aliasExists = this.db.prepare(
            "SELECT id, answer FROM faq WHERE device_id = ? AND question_norm = ?"
          ).get(s.device, aliasNorm);
          if (!aliasExists) {
            insert.run(s.device, alias, aliasNorm, s.answer, s.topic || null);
          } else if (aliasExists.answer !== s.answer) {
            update.run(s.answer, s.device, aliasNorm);
          }
          if (s.topic) {
            setTopic.run(s.topic, s.device, aliasNorm);
          }
        }
      }
    }
  ...
  _loadCache() {
    const rows = this.db.prepare("SELECT id, device_id, question, question_norm, answer, topic FROM faq").all();
    this.cache.clear();
    for (const row of rows) {
  ...
  match(deviceId, userQuestion) {
    ...
    if (bestScore >= this.threshold && bestEntry) {
      ...
      const matchType = bestScore >= 0.99 ? "match" : "similarity";
      return { answer: bestEntry.answer, score: bestScore, faqId: bestEntry.id, topic: bestEntry.topic, matchType };
    }
  ...
  logRequest(deviceId, question, source, offTopic = 0, scores = null) {
    this.db.prepare(`
      INSERT INTO request_log (device_id, question, source, off_topic, tri_score, kw_score, best_score, matched_q)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(deviceId, question, source, offTopic ? 1 : 0,
      scores ? scores.tri : null, scores ? scores.kw : null, scores ? scores.best : null, scores ? scores.matchedQ : null);
  }

  getRecentRequests(limit = 50, deviceId = null) {
    if (deviceId) {
      return this.db.prepare(`
        SELECT device_id, question, source, off_topic, tri_score, kw_score, best_score, matched_q, created_at
  ...
  _similarityDetail(a, b) {
    ...
    if (kwA.size === 0 || kwB.size === 0) return { tri: triScore, kw: 0, best: triScore };
    ...
    return { tri: triScore, kw: kwScore, best: Math.max(triScore, kwScore) };
  }

  _similarity(a, b) {
    return this._similarityDetail(a, b).best;
  }

  scoreBest(deviceId, userQuestion) {
    const norm = normalize(userQuestion);
    const entries = this.cache.get(deviceId);
    if (!entries || entries.length === 0) return { tri: 0, kw: 0, best: 0, matchedQ: null };

    let bestDetail = { tri: 0, kw: 0, best: 0 };
    let bestEntry = null;

    for (const entry of entries) {
      const detail = this._similarityDetail(norm, entry.question_norm);
      if (detail.best > bestDetail.best) {
        bestDetail = detail;
        bestEntry = entry;
      }
    }

    return { tri: bestDetail.tri, kw: bestDetail.kw, best: bestDetail.best, matchedQ: bestEntry ? bestEntry.question : null };
  }
}
